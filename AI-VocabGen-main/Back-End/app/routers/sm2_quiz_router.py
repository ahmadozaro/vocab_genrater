...
def _is_due(word: Word, now: datetime | None = None) -> bool:
    ...
    return word.status != "pending" and (word.next_review_at is None or word.next_review_at <= now)
...
def _select_sm2_words(words: list[Word], limit: int = MAX_SM2_QUESTIONS) -> list[Word]:
    now = datetime.utcnow()
    active_words = [w for w in words if w.status != "pending"]
    selected: list[Word] = []
    seen: set[int] = set()
...
def submit_sm2_quiz(
    ...
    for item in items:
        ...
        word = db.query(Word).filter(Word.id == item.userWordId, Word.user_id == current_user.id).first()
        if not word:
            continue
