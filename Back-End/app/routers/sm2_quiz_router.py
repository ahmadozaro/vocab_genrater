...
def _update_streak(
    ...
    progress.completed_sm2_quizzes = 1
...
def start_sm2_quiz(
    ...
    for i, word in enumerate(selected_words):
        ...
        if qtype == "tf":
            ...
            question_text, opts, correct = generate_fill(word, words)
...
def submit_sm2_quiz(
    ...
    pending_updates: list[tuple[Word, object]] = []
    ...
    for item in items:
        ...
        pending_updates.append((word, snapshot))
    ...
    for word, snapshot in pending_updates:

        word.sm2EaseFactor = snapshot.new_ease_factor

        word.sm2Repeats = snapshot.new_repeats

        word.sm2IntervalDays = snapshot.new_interval_days

        word.nextReviewDate = snapshot.new_next_review_at

        word.lastReviewedAt = now

        word.score = snapshot.new_score

        word.correctStreak = snapshot.new_correct_streak

        word.wrongStreak = snapshot.new_wrong_streak

        word.status = snapshot.status
