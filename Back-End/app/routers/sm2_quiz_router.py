...
from app.services.sm2_service import apply_modified_sm2, infer_quality, normalize_answer
...
def _is_due(word: Word, now: datetime | None = None) -> bool:
    ...
    return bool(word.is_active) and word.status != "pending" and (word.next_review_at is None or word.next_review_at <= now)
...
def _select_sm2_words(words: list[Word], limit: int = MAX_SM2_QUESTIONS) -> list[Word]:
    ...
    groups = [
        sorted([w for w in active_words if _is_due(w, now)], key=lambda w: (-_overdue_days(w, now), w.score or 0)),
        ...
        sorted([w for w in active_words if w.status in ("new", "learning")], key=lambda w: w.id),
    ...
    for group in groups:
        for word in group:
            ...
            seen.add(word.id)
            if len(selected) >= min(limit, len(active_words)):
                return selected
    return selected


def _build_options(correct: str, all_words: list[Word], tested_word_id: int) -> list[str]:
    distractors = [w.text for w in all_words if w.id != tested_word_id and w.text]
    fallback = ["learn", "practice", "memory", "example", "review", "answer"]
    pool = []
    for item in distractors + fallback:
        if normalize_answer(item) != normalize_answer(correct) and item not in pool:
            pool.append(item)
    selected = random.sample(pool, k=min(3, len(pool)))
    while len(selected) < 3:
        selected.append(f"Option {len(selected) + 1}")
    options = selected[:3] + [correct]
    random.shuffle(options)
    return options


def _generate_fill_in_blank_sm2(word: Word) -> tuple[str, list[str], str]:
    correct = word.text
...
def _generate_true_false_sm2(word: Word, all_texts: list[str]) -> tuple[str, list[str], str]:
    correct = word.text
    meaning = word.arabicMeaning or word.translationAr or "this word"
    is_true = random.choice([True, False])
    if is_true:
        display_text = correct
        statement = f'Is the word "{display_text}" correctly defined as "{meaning}"?'
    else:
...
def _update_streak(db: Session, user_id: int, has_real_answer: bool) -> tuple[bool, int]:
    ...
    progress.completed_sm2_quizzes = (progress.completed_sm2_quizzes or 0) + 1
    return True, progress.daily_streak or 0
...
def get_sm2_due_summary(
    ...
    now = datetime.utcnow()
    words = db.query(Word).filter(Word.user_id == current_user.id, Word.is_active == 1).all()
...
def start_sm2_quiz(
    ...
    if not selected_words:
        raise HTTPException(status_code=400, detail="No active words available for SM2 review")
    ...
    for i, word in enumerate(selected_words):
        ...
        if r == 1:
            question_text, opts, _ = _generate_fill_in_blank_sm2(word)
            options = opts
            qtype = "fill"
        elif r == 2:
            question_text, opts, correct = _generate_true_false_sm2(word, all_texts)
            options = opts
            qtype = "tf"
        else:
            meaning = word.arabicMeaning or word.translationAr or "this word"
            question_text = f"What is the English word for: {meaning}?"
            options = _build_options(correct, words, word.id)
            qtype = "mcq"
        item = SM2QuizItem(
            quizId=quiz.id,
            userWordId=word.id,
    ...
    return SM2StartResponse(quizId=quiz.id, questions=[_item_response(item) for item in created_items])
...
def submit_sm2_quiz(
    ...
    quiz = db.query(SM2Quiz).filter(SM2Quiz.id == quiz_id, SM2Quiz.userId == current_user.id).first()
    if not quiz:
        raise HTTPException(status_code=404, detail="SM2 quiz not found")
    if quiz.status != "in_progress":
        raise HTTPException(status_code=400, detail="This SM2 quiz was already submitted or abandoned")
    ...
    for item in items:
        ...
        word = db.query(Word).filter(Word.id == item.userWordId, Word.user_id == current_user.id, Word.is_active == 1).first()
        if not word:
            continue
