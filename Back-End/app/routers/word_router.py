...
def create_word(
    ...
    manual_definition = _clean_text(word.definition)

    if manual_definition:

        dictionary_word.definition_en = manual_definition

    elif ai_lookup.get("definition") and not dictionary_word.definition_en:
...
def get_words(
    ...
    activated = _try_activate_pending_words(db, current_user.id)
